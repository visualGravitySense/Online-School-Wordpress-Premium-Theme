...
if ( ! function_exists( 'wordpress_posted_by' ) ) :
	/**
	 * Prints HTML with meta information for the current author.
	 */
	function wordpress_posted_by() {
		$byline = sprintf(
...
if ( ! function_exists( 'wordpress_entry_footer' ) ) :
	...
	function wordpress_entry_footer() {
		...
		if ( 'post' === get_post_type() ) {
			...
			/* translators: used between list items, there is a space after the comma */
			$tags_list = get_the_tag_list( '', esc_html_x( ', ', 'list item separator', 'wordpress' ) );
			if ( $tags_list ) {
			...
			comments_popup_link(
				sprintf(
					wp_kses(
						...
						__( 'Leave a Comment<span class="screen-reader-text"> on %s</span>', 'wordpress' ),
						array(
							'span' => array(
								'class' => array(),
							),
